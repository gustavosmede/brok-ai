...
test("ships the Brok.ai product instead of the starter preview", async () => {
  ...
  assert.match(page, /Nenhuma ordem real pode sair daqui/);
  ...
  assert.match(page, /ATIVO RESOLVIDO/);
  assert.match(page, /Alternativas encontradas/);
  assert.match(page, /Não é recomendação/);
  assert.match(page, /\/api\/position-detail/);
  assert.match(drawer, /POSITION &lt;GO&gt; \/\/ DETAIL/);
  assert.match(drawer, /aria-modal="true"/);
  assert.match(drawer, /Histórico completo do ticker/);
  assert.match(drawer, /As ações abrem o preview obrigatório/);
  assert.match(drawer, /Últimas notícias sobre/);
  assert.match(drawer, /Abrir gráfico no TradingView/);
