...
function money(cents: number | null, sign = false): string {
  ...
  const formatted = new Intl.NumberFormat("pt-BR", { style: "currency", currency: "USD", minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Math.abs(cents) / 100);
...
function shares(micros: number): string {
  return new Intl.NumberFormat("pt-BR", { maximumFractionDigits: 8 }).format(micros / 1_000_000);
}
...
function PositionChart({ detail }: { detail: PositionDetail }) {
  ...
  return <div className="position-chart-wrap">
    <div className="position-chart-toolbar"><div><span><i className="price-key" />Preço</span><span><i className="cost-key" />Preço médio</span><span><i className="stop-key" />Stop</span><span><i className="target-key" />Alvo</span></div><div>{([30, 90, 365] as const).map((days) => <button type="button" className={period === days ? "active" : ""} key={days} onClick={() => setPeriod(days)}>{days === 30 ? "1M" : days === 90 ? "3M" : "1A"}</button>)}</div></div>
    {bars.length ? <canvas ref={canvasRef} aria-label={`Histórico de preço de ${detail.symbol} com preço médio, stop e alvo`} /> : <div className="position-chart-empty"><strong>Histórico indisponível</strong><span>{detail.history.error ?? "O restante da posição continua disponível."}</span></div>}
  </div>;
}
...
function PositionPerformanceChart({ detail }: { detail: PositionDetail }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const formatPnl = (value: number) => `${value > 0 ? "+" : value < 0 ? "−" : ""}${new Intl.NumberFormat("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Math.abs(value) / 100)}`;
  ...
  return <div className="position-chart-wrap position-performance-chart">
    <div className="position-chart-toolbar"><div><span><i className={last && last.pnlCents < 0 ? "pnl-key negative" : "pnl-key"} />P&amp;L da posição</span>{last ? <strong className={tone(last.pnlCents)}>{money(last.pnlCents, true)} · {percent(last.returnPct, true)}</strong> : null}</div><div>{([30, 90, 365] as const).map((days) => <button type="button" className={period === days ? "active" : ""} key={days} onClick={() => setPeriod(days)}>{days === 30 ? "1M" : days === 90 ? "3M" : "1A"}</button>)}</div></div>
    {points.length ? <canvas ref={canvasRef} aria-label={`Curva de P&L da posição ${detail.symbol} ao longo do tempo`} /> : <div className="position-chart-empty"><strong>Performance ainda indisponível</strong><span>A curva começa no primeiro fill do ciclo aberto.</span></div>}
  </div>;
}
...
export function PositionDetailDrawer({ detail, loading, error, onClose, onReduce, onClosePosition }: {
  ...
  return <div className="position-drawer-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside ref={drawerRef} className="position-drawer" role="dialog" aria-modal="true" aria-labelledby="position-detail-title">
      <header className="position-drawer-header"><div><p>POSITION &lt;GO&gt; // DETAIL</p><h2 id="position-detail-title">{detail?.symbol ?? "Carregando posição"}</h2>{detail ? <span>{detail.direction} · {detail.name} · {detail.assetClass} · {detail.exchange}</span> : null}</div><button ref={closeRef} type="button" onClick={onClose} aria-label="Fechar detalhe da posição">×</button></header>
      {loading ? <div className="position-drawer-state"><span className="spin">↻</span><strong>Reconciliando posição</strong><p>Carregando histórico, ordens e fills do ativo.</p></div> : error ? <div className="position-drawer-state error"><span>!</span><strong>Não foi possível abrir a posição</strong><p>{error}</p><button type="button" onClick={onClose}>Voltar</button></div> : detail ? <>
        ...
        <nav className="position-detail-tabs" aria-label="Seções do detalhe da posição">{([['overview', '1 Visão geral'], ['performance', '2 Performance'], ['risk', '3 Risco'], ['execution', '4 Execução']] as const).map(([key, label]) => <button type="button" key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}>{label}</button>)}</nav>
        <div className="position-drawer-body">
          {tab === "overview" ? <>
            <section className="position-detail-metrics"><div><span>VALOR DE MERCADO</span><strong>{money(detail.position.marketValueCents)}</strong><small>{percent(detail.position.allocationPct)} DO PL</small></div><div><span>CUSTO BASE</span><strong>{money(detail.position.costBasisCents)}</strong><small>PX MÉDIO {priceMoney(detail.position.averageCostCents)}</small></div><div><span>P&amp;L ABERTO</span><strong className={tone(detail.pnl.unrealizedCents)}>{money(detail.pnl.unrealizedCents, true)}</strong><small className={tone(detail.pnl.unrealizedPct)}>{percent(detail.pnl.unrealizedPct, true)}</small></div><div><span>QUANTIDADE · {detail.direction}</span><strong>{shares(Math.abs(detail.position.quantityMicros))}</strong><small>{detail.cycle.daysHeld === null ? "CICLO INDEFINIDO" : `${detail.cycle.daysHeld} DIAS`}</small></div></section>
            <PositionChart detail={detail} />
            <section className="position-facts"><div><span>Abertura do ciclo</span><strong>{dateTime(detail.cycle.openedAt)}</strong></div><div><span>Preço de equilíbrio</span><strong>{money(detail.risk.breakEvenCents)}</strong></div><div><span>Contribuição ao P&amp;L</span><strong className={tone(detail.pnl.contributionPct)}>{percent(detail.pnl.contributionPct, true)}</strong></div><div><span>P&amp;L total</span><strong className={tone(detail.pnl.totalCents)}>{money(detail.pnl.totalCents, true)}</strong></div></section>
          </> : null}
          {tab === "performance" ? <>
            <section className="position-detail-metrics"><div><span>P&amp;L DO DIA</span><strong className={tone(detail.pnl.dayCents)}>{money(detail.pnl.dayCents, true)}</strong><small className={tone(detail.pnl.dayPct)}>{percent(detail.pnl.dayPct, true)}</small></div><div><span>NÃO REALIZADO</span><strong className={tone(detail.pnl.unrealizedCents)}>{money(detail.pnl.unrealizedCents, true)}</strong><small>{percent(detail.pnl.unrealizedPct, true)}</small></div><div><span>REALIZADO HIST.</span><strong className={tone(detail.pnl.realizedHistoricalCents)}>{money(detail.pnl.realizedHistoricalCents, true)}</strong><small>VIDA DO TICKER</small></div><div><span>TOTAL</span><strong className={tone(detail.pnl.totalCents)}>{money(detail.pnl.totalCents, true)}</strong><small>REAL. + NÃO REAL.</small></div></section>
            <PositionPerformanceChart detail={detail} />
            <p className="position-method-note">O realizado é histórico vitalício do ticker. O retorno aberto compara o valor atual ao custo base do ciclo em aberto.</p>
          </> : null}
          {tab === "risk" ? <>
            <section className="position-detail-metrics"><div><span>STOP</span><strong className={detail.risk.stopPriceCents ? "" : "negative"}>{money(detail.risk.stopPriceCents)}</strong><small>{detail.risk.stopDistancePct === null ? "SEM PROTEÇÃO" : `${percent(detail.risk.stopDistancePct)} ${detail.direction === "SHORT" ? "ACIMA" : "ABAIXO"}`}</small></div><div><span>ALVO</span><strong>{money(detail.risk.targetPriceCents)}</strong><small>{detail.risk.targetDistancePct === null ? "SEM ALVO" : `${percent(detail.risk.targetDistancePct)} ${detail.direction === "SHORT" ? "ABAIXO" : "ACIMA"}`}</small></div><div><span>CAPITAL EM RISCO</span><strong className={detail.risk.capitalAtRiskCents === null ? "negative" : ""}>{detail.risk.capitalAtRiskCents === null ? "NÃO LIMITADO" : money(detail.risk.capitalAtRiskCents)}</strong><small>ATÉ O STOP</small></div><div><span>REWARD / RISK</span><strong>{detail.risk.rewardRiskRatio === null ? "—" : `${detail.risk.rewardRiskRatio.toFixed(2)}x`}</strong><small>ALVO VS STOP</small></div></section>
            <section className="position-scenarios"><div className="position-section-title"><span>STRESS TEST</span><strong>Cenários lineares da posição</strong></div>{detail.risk.scenarios.map((scenario) => <div key={scenario.shockPct}><span className={tone(scenario.shockPct)}>{scenario.shockPct > 0 ? "+" : ""}{scenario.shockPct}%</span><strong className={tone(scenario.pnlCents)}>{money(scenario.pnlCents, true)}</strong><em>{money(scenario.resultingValueCents)} valor resultante</em></div>)}</section>
          </> : null}
          {tab === "execution" ? <>
            <section className="position-execution-section"><div className="position-section-title"><span>FILLS</span><strong>Histórico completo do ticker</strong></div><div className="position-mini-table"><div className="head"><span>Lado</span><span>Quantidade</span><span>Preço</span><span>Notional</span><span>Data</span></div>{detail.fills.map((fill) => <div key={fill.id}><span className={fill.side === "BUY" ? "positive" : "negative"}>{fill.side}</span><span>{shares(fill.quantity_micros)}</span><span>{money(fill.price_cents)}</span><span>{money(Math.round(fill.quantity_micros * fill.price_cents / 1_000_000))}</span><span>{dateTime(fill.created_at)}</span></div>)}</div></section>
            <section className="position-execution-section"><div className="position-section-title"><span>ORDENS</span><strong>Entradas, reduções e proteção OCO</strong></div><div className="position-order-list">{detail.orders.map((order) => <div key={order.id}><span className={`status-badge ${order.status.toLowerCase()}`}>{order.status}</span><strong>{order.side} · {order.order_type}</strong><small>{order.role.replaceAll("_", " ")} · {order.trigger_price_cents ? money(order.trigger_price_cents) : "Mercado"} · {dateTime(order.created_at)}</small></div>)}</div></section>
          ...
          <section className="position-news-section" aria-labelledby="position-news-title">
            <div className="position-news-heading"><div><span>NEWS // YAHOO</span><h3 id="position-news-title">Últimas notícias sobre {detail.symbol}</h3><p>Ordenadas por relação com o ticker e recência.</p></div><a href={detail.tradingViewUrl} target="_blank" rel="noopener noreferrer">Abrir gráfico no TradingView <b>↗</b></a></div>
            {detail.news.length ? <div className="position-news-list">{detail.news.map((article) => <a key={article.id} href={article.url} target="_blank" rel="noopener noreferrer"><span className={article.priority ? "highlight" : ""}>{article.priority ? "DESTAQUE" : "RECENTE"}</span><div><strong>{article.title}</strong><small>{article.publisher} · {dateTime(article.publishedAt)}</small></div><b aria-hidden="true">↗</b></a>)}</div> : <div className="position-news-empty"><strong>Sem notícias recentes disponíveis</strong><span>{detail.newsError ?? "O Yahoo não retornou matérias relacionadas a este ticker."}</span></div>}
            <p className="position-news-disclaimer">Links externos apenas para informação. O Brok.ai não resume nem altera as manchetes e não as utiliza como recomendação.</p>
          </section>
        </div>
        <footer className="position-drawer-actions"><button type="button" onClick={() => onReduce(detail.symbol, 25)}>Reduzir 25%</button><button type="button" onClick={() => onReduce(detail.symbol, 50)}>Reduzir 50%</button><button type="button" className="danger" onClick={() => onClosePosition(detail.symbol)}>Fechar posição</button><span>As ações abrem o preview obrigatório.</span></footer>
      </> : null}
    </aside>
  </div>;
}
