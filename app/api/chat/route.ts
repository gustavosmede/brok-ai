...
export async function POST(request: Request) {
  try {
    ...
    if (!message) return Response.json({ error: "Write an order" }, { status: 400 });
    const parsed = await parseIntentWithOllama(message, payload.model?.trim() || undefined);
    return Response.json(parsed);
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Failed to interpret the order" }, { status: 400 });
  }
}
