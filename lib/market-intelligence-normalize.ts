...
const geopoliticalTerms = ["attack", "ceasefire", "china", "conflict", "gaza", "geopolit", "iran", "israel", "military", "missile", "nato", "red sea", "russia", "sanction", "tariff", "taiwan", "trade war", "ukraine", "war", "opec"];
const urgentTerms = ["breaking", "flash", "alert", "just in", "emergency", "unexpectedly", "halts trading", "trading halted", "trading suspension"];
const systemicTerms = ["interest rate decision", "rate decision", "fomc", "central bank", "cpi", "consumer price", "inflation", "nonfarm payroll", "payrolls", "unemployment rate", "default", "bankruptcy", "bank failure", "capital controls", "ceasefire", "invasion", "airstrike", "missile", "sanction", "tariff", "embargo", "attack", "sovereign crisis", "rate shock", "inflation shock"];
const companyMovingTerms = ["earnings", "guidance", "profit warning", "acquisition", "merger", "takeover", "dividend cut", "dividend suspension", "fraud", "investigation", "downgrade", "forecast change", "strategic review", "asset sale", "spin-off"];
...
export function classifyNews(title: string, description = "", itemLabels: string[] = []): NewsCategory {
  const haystack = `${title} ${description} ${itemLabels.join(" ")}`.toLocaleLowerCase("en-US");
...
export function assessNewsImpact(item: { title: string; description?: string; labels?: string[]; source: NewsSource; category: NewsCategory; portfolioRelated?: boolean }): NewsImpact {
  const haystack = `${item.title} ${item.description ?? ""} ${(item.labels ?? []).join(" ")}`.toLocaleLowerCase("en-US");
  let score = 0;
